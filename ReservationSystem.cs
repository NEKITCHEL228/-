...
public class ReservationSystem
{
    ...
    public bool RemoveReservation(int reservationId)
    {
        ...
        var table = tables.Find(t => t.GetID() == reservation.GetTableID());
        if (table != null)
        {
    ...
    public void PrintAllReservationsID()
    {
        Console.Write("ID бронирований: [ ");
        foreach (Reservation r in reservations)
        {
            Console.Write(r.GetID() + "; ");
        }
        Console.Write("]");
        Console.WriteLine();
...
public void CreateReservations(int n)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        Table table = tables.FirstOrDefault(t => t.GetID() == TableID);
        if (table == null)
        {
...
{
    Console.Clear();
    var table = tables.Find(t => t.GetID() == TableID);
    if (table == null)
    {
...
public void PrintInfoAboutClientReservations(string Name, string PhoneNumberLast_4)
{
    ...
    foreach (Reservation reservation in reservations)
    {
        ...
        {
            string[] SplittedNumber = reservation.GetPhoneNumber().Split("-");
